...
export default defineCommand({
   ...
   async run(ctx) {
      ...
      if (!templates[template]) {
         consola.error(
        `Template ${template} is not supported. Possible values: ${Object.keys(templates).join(', ')}`,
         )
         process.exit(1)
      }
      ...
      // Recognize the project type (Nuxt or Vite)
      const isNuxt = existsSync(resolve(cwd, 'nuxt.config.js')) || existsSync(resolve(cwd, 'nuxt.config.ts'))

      // Loading the configuration for Nuxt projects only
      let basePath = cwd
      if (isNuxt) {
         const kit = await loadKit(cwd)
         const config = await kit.loadNuxtConfig({ cwd })
         basePath = config.srcDir || cwd
      }

      // Check whether a 'src' folder exists
      if (!isNuxt && existsSync(resolve(cwd, 'src')))
         basePath = resolve(cwd, 'src')

      // Resolve template
      const res = templates[template]({ name, args: ctx.args })

      // Resolve full path to generated file
      const path = resolve(basePath, res.path)
